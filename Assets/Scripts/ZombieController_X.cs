...
public class ZombieController_X : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        Debug.Log("x:" + (this.gameObject.transform.position.x - basePostion));

        //Transformのxの値が一定値を超えたときに向きを反対にする
        if (this.gameObject.transform.position.x - basePostion > max_x || this.gameObject.transform.position.x - basePostion < (-max_x))
        {
            speed *= -1;
